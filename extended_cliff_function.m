...
%
% function [func,grad_func] = extended_cliff_function(x);
%
...
func = 0;
for i = 1 : m
   func = func +  ((x(2*i-1)-3)/100)^2 - (x(2*i-1) - x(2*i)) +...
...
if nargout > 1
    ...
    grad_func = zeros(n,1);
    for i = 1 : m
        grad_func(2*i-1) = ((x(2*i-1)-3)/100)/50 - 1 + ...
            20*exp(20*(x(2*i-1)-x(2*i)));
        grad_func(2*i) = 1 - 20*exp(20*(x(2*i-1)-x(2*i)));
    end
end 
