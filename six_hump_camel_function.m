...
%
% function [func,grad_func] = six_hump_camel_function(x);
%
...
%
function [func,grad_func] = six_hump_camel_function(x)
func = (4-2.1*x(1)^2+x(1)^4/3)*x(1)^2+x(1)*x(2)+(-4+4*x(2)^2)*x(2)^2;

% Compute the gradient of the objective function at x.
if nargout > 1
    n = length(x);
    grad_func = zeros(n,1);
    grad_func(1) = 8*x(1)-8.4*x(1)^3+2*x(1)^5+x(2);
    grad_func(2) = x(1)-8*x(2)+16*x(2)^3;
end
